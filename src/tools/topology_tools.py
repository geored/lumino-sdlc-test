"""
Topology mapping tools — extracted from server-mcp.py.

Contains `_process_namespace_topology` (internal helper) and
`live_system_topology_mapper_impl` (the core implementation, called by the
@mcp.tool() wrapper that remains in server-mcp.py).
"""
...
async def _process_namespace_topology(
    ...
    permissions: Dict[str, Any] = {"accessible": [], "denied": [], "errors": []}
                ...
                for deployment in deployments.items:
                    ...
                    if include_metrics:
                        node["metrics"] = await get_resource_metrics(
                            cluster_name, "deployment", namespace,
                            deployment.metadata.name, logger,
                        )
                    nodes.append(node)
                    stats["nodes"] += 1
            ...
            try:
                ...
                for replicaset in replicasets.items:
                    ...
                    node = {
                        ...
                        "cluster": cluster_name,
                        "status": (
                            "Active" if (replicaset.status.ready_replicas or 0) > 0
                            else "Inactive"
                        ),
                        "metadata": {
                    ...
                    if include_metrics:
                        node["metrics"] = await get_resource_metrics(
                            cluster_name, "replicaset", namespace,
                            replicaset.metadata.name, logger,
                        )
                    nodes.append(node)
                    stats["nodes"] += 1
        ...
        if "services" in component_types:
            try:
                ...
                for service in services.items:
                    ...
                    if include_metrics:
                        node["metrics"] = await get_resource_metrics(
                            cluster_name, "service", namespace,
                            service.metadata.name, logger,
                        )
                    nodes.append(node)
                    stats["nodes"] += 1
                    service_dict = service.to_dict()
                    service_edges = await analyze_service_dependencies(
                        service_dict, cluster_name, core_api, logger,
                        pods_list=pods_list,
                    )
        ...
        if "persistentvolumeclaims" in component_types:
            try:
                pvcs = await asyncio.to_thread(
                    core_api.list_namespaced_persistent_volume_claim, namespace=namespace,
                )
                for pvc in pvcs.items:
                    node_id = generate_node_id(
                        cluster_name, namespace, "persistentvolumeclaim", pvc.metadata.name,
                    )
                    node = {
                        "id": node_id,
                    ...
                    if include_metrics:
                        node["metrics"] = await get_resource_metrics(
                            cluster_name, "persistentvolumeclaim", namespace,
                            pvc.metadata.name, logger,
                        )
                    nodes.append(node)
                    stats["nodes"] += 1
            except Exception as e:
                error_info = handle_resource_fetch_error(
                    e, "persistentvolumeclaims", namespace, skip_on_permission_denied, logger,
                )
                if error_info["permission_denied"]:
        ...
        if "pipelineruns" in component_types:
            try:
                pipeline_runs = await asyncio.to_thread(
                    ...
                    namespace=namespace, plural="pipelineruns", limit=200,
                )
                for pr in pipeline_runs.get("items", []):
                    pr_name = pr.get("metadata", {}).get("name", "")
                    node_id = generate_node_id(cluster_name, namespace, "pipelinerun", pr_name)
                    node = {
                        "id": node_id,
                        "type": "pipelinerun",
                        "name": pr_name,
                    ...
                    if include_metrics:
                        node["metrics"] = await get_resource_metrics(
                            cluster_name, "pipelinerun", namespace, pr_name, logger
                        )
                    nodes.append(node)
                    stats["nodes"] += 1
                    pipeline_ref = pr.get("spec", {}).get("pipelineRef", {}).get("name")
        ...
        if "pipelines" in component_types:
            try:
                pipelines = await asyncio.to_thread(
                    custom_api.list_namespaced_custom_object,
                    group="tekton.dev", version="v1",
                    namespace=namespace, plural="pipelines",
                )
                for pipeline in pipelines.get("items", []):
                    pipeline_name = pipeline.get("metadata", {}).get("name", "")
                    node_id = generate_node_id(cluster_name, namespace, "pipeline", pipeline_name)
                    node = {
                        "id": node_id,
                        "type": "pipeline",
                        "name": pipeline_name,
        ...
        if "taskruns" in component_types:
            try:
                task_runs = await asyncio.to_thread(
                    custom_api.list_namespaced_custom_object,
                    group="tekton.dev", version="v1",
                    namespace=namespace, plural="taskruns", limit=500,
                )
        ...
        if "tasks" in component_types:
            try:
                tasks = await asyncio.to_thread(
                    custom_api.list_namespaced_custom_object,
                    group="tekton.dev", version="v1",
                    namespace=namespace, plural="tasks",
                )
...
async def live_system_topology_mapper_impl(
    ...
    """
    Core implementation of live_system_topology_mapper — accepts injected API clients.

    Generate real-time dependency graph of Kubernetes/Tekton components and their
    interconnections. Maps Services, Deployments, Pipelines, PVCs, and their
    relationships via ownerReferences and selectors.
    """
    ...
    try:
        ...
        if not component_types:
            component_types = [
                ...
                "pipelineruns", "pipelines", "taskruns", "tasks",
            ]

        nodes: List[Dict[str, Any]] = []
        edges: List[Dict[str, Any]] = []
        cluster_stats: Dict[str, Any] = {}
        permissions_report: Dict[str, Any] = {"accessible": [], "denied": [], "errors": []}
        ...
        logger.info(
            f"Topology mapping completed: {total_nodes} nodes, {total_edges} edges "
            f"across {clusters_mapped} clusters in {execution_time:.2f}s"
        )
