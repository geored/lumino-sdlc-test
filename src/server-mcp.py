...
async def _execute_prometheus_query_internal(
    ...
    """
    try:
        prometheus_url, endpoint_type = await discover_prometheus_endpoint(k8s_core_api=k8s_core_api, k8s_custom_api=k8s_custom_api)
...
async def prometheus_query(
    ...
    try:
        ...
        prometheus_url, endpoint_type = await discover_prometheus_endpoint(cluster_override=cluster, k8s_core_api=k8s_core_api, k8s_custom_api=k8s_custom_api)
        if not prometheus_url:
            return {
                "status": "error",
