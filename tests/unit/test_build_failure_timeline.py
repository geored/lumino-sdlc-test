...
def test_datetime_timestamp_converted_to_isoformat():
    ...
    events = [{"timestamp": dt, "reason": "Started", "event_string": "pod started", "severity": "low"}]
    result = _run(
...
def test_string_timestamp_preserved():
    events = [{"timestamp": "2024-06-01T12:00:00Z", "reason": "R", "event_string": "e", "severity": "low"}]
    result = _run(
        build_failure_timeline("ns", "id", 1, _make_events_func(events), _FakeLogger())
    )
...
def test_none_timestamp_stored_as_none():
    events = [
        {"timestamp": None, "reason": "NoTS", "event_string": "no ts event", "severity": "medium"},
        {"reason": "Missing", "event_string": "missing key event", "severity": "medium"},
    ]
    result = _run(
        build_failure_timeline("ns", "id", 1, _make_events_func(events), _FakeLogger())
    )
    timestamps = [e["timestamp"] for e in result]
    assert all(ts is None for ts in timestamps), "Expected all None, got: {}".format(timestamps)
...
def test_sort_with_mixed_none_and_string_timestamps_no_type_error():
    ...
    events = [
        ...
        {"timestamp": "2024-06-01T09:00:00Z", "reason": "StrTS", "event_string": "c", "severity": "low"},
    ]
...
def test_none_timestamps_pushed_to_end():
    ...
    events = [
        {"timestamp": None,    "reason": "NoTS",  "event_string": "no ts",  "severity": "low"},
        {"timestamp": dt_early, "reason": "Early", "event_string": "early",  "severity": "low"},
        {"timestamp": dt_late,  "reason": "Late",  "event_string": "late",   "severity": "low"},
    ]
    result = _run(
        build_failure_timeline("ns", "id", 1, _make_events_func(events), _FakeLogger())
    )
    ...
    assert result[-1]["timestamp"] is None, \
        "Expected None-timestamp entry last, but got: {}".format([e["timestamp"] for e in result])
...
def test_result_capped_at_ten_entries():
    events = [
        {"timestamp": "2024-06-01T{:02d}:00:00Z".format(i), "reason": "R", "event_string": "e", "severity": "low"}
        for i in range(15)
    ]
    result = _run(
        build_failure_timeline("ns", "id", 1, _make_events_func(events), _FakeLogger())
