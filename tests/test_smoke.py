...
def test_constants_importable():
    ...
    try:
        ...
        raise AssertionError(f"constants.py failed to import: {exc}") from exc

    for name in ("SMART_EVENTS_CONFIG", "LOG_ANALYSIS_CONFIG"):
        assert hasattr(mod, name), f"constants.py missing expected symbol: {name}"
...
def test_main_uses_importlib():
    ...
    assert "importlib" in source, (
        "main.py must use importlib to load src/server-mcp.py "
        "(hyphen in filename prevents normal import)"
    )
