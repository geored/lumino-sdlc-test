...
def test_constants_importable():
    ...
    try:
        ...
        raise AssertionError("constants.py failed to import: " + str(e))
...
def test_main_uses_importlib():
    ...
    with open("main.py", "r") as f:
        source = f.read()
    assert "importlib" in source, "main.py must use importlib to load src/server-mcp.py (hyphen in filename prevents normal import)"
    assert "import server-mcp" not in source, "main.py must not use 'import server-mcp' -- use importlib instead"
